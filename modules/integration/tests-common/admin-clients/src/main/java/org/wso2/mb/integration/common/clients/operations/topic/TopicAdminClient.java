...
public class TopicAdminClient {
    ...
    public TopicNode getTopicByName(String topic) {

        TopicNode[] topicNodes = new TopicNode[0];

        try {
            topicNodes = stub.getAllTopics().getChildren();
        } catch (RemoteException e) {
            log.error("getTopicByName failed : " + e.getMessage());
        } catch (TopicManagerAdminServiceEventAdminExceptionException e) {
            log.error("getTopicByName failed : " + e.getMessage());
        }
